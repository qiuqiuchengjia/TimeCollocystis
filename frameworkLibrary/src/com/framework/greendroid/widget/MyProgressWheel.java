...
public class MyProgressWheel extends View {
	private static final String LOGTAG = LogUtil
			.makeLogTag(MyProgressWheel.class);
	/**
	 * ********* DEFAULTS * **********
	 */
	// Sizes (with defaults in DP)
	private int circleRadius = 28;
	private int circlePadding = 4;
	...
	Paint bitmap_heartPaint = new Paint();
	...
	private Bitmap centerPic = null;
	private int centerPicId = 0;
	...
	private void setupPaints() {
		...
		bitmap_heartPaint.setStyle(Paint.Style.FILL);// 充满
		bitmap_heartPaint.setAntiAlias(true);// 设置画笔的锯齿效果
	}
	...
	private void parseAttributes(TypedArray a) {
		...
		circlePadding = (int) a.getDimension(
				R.styleable.ProgressWheels_circlePadding, circlePadding);
		...
		centerPicId = a.getResourceId(R.styleable.ProgressWheels_centerPic, 0);
		...
		BitmapDrawable bmpDraw = (BitmapDrawable) r.getDrawable(centerPicId);
		centerPic = bmpDraw.getBitmap();
		// Recycle
		a.recycle();
	}
	...
	protected void drawCenter(Canvas canvas) {
		...
		left = (viewWidth - circleRadius) / 2 + circlePadding;
		top = (viewHeight - circleRadius) / 2 + circlePadding;
		right = (viewWidth + circleRadius) / 2 - circlePadding;
		bottom = (viewHeight + circleRadius) / 2 - circlePadding;
		// left = circleBounds.left - 4;
		// top = circleBounds.top - 4;
		// right = circleBounds.right - 4;
		// bottom = circleBounds.bottom - 4;

		RectF center = new RectF(left, top, right, bottom);
		canvas.drawBitmap(centerPic, null, center, bitmap_heartPaint);
	}
