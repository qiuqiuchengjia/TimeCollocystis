...
public class CircleFlowIndicator extends View implements FlowIndicator,
	...
	public CircleFlowIndicator(Context context, AttributeSet attrs) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.CircleViewFlowIndicator);

		// Gets the inactive circle type, defaulting to "fill"
		int activeType = a.getInt(R.styleable.CircleViewFlowIndicator_activeType,
				STYLE_FILL);
		...
		int activeColor = a
				.getColor(R.styleable.CircleViewFlowIndicator_activeColor,
						activeDefaultColor);

		// Gets the inactive circle type, defaulting to "stroke"
		int inactiveType = a.getInt(
				R.styleable.CircleViewFlowIndicator_inactiveType, STYLE_STROKE);

		int inactiveDefaultColor = 0x44FFFFFF;
		// Get a custom inactive color if there is one
		int inactiveColor = a.getColor(
				R.styleable.CircleViewFlowIndicator_inactiveColor,
				inactiveDefaultColor);

		// Retrieve the radius
		radius = a.getDimension(R.styleable.CircleViewFlowIndicator_radiuss, 4.0f);

		circleSeparation = a.getDimension(
				R.styleable.CircleViewFlowIndicator_circleSeparation, 2 * radius
						+ radius);
		activeRadius = a.getDimension(
				R.styleable.CircleViewFlowIndicator_activeRadius, 0.5f);
		// Retrieve the fade out time
		fadeOutTime = a.getInt(R.styleable.CircleViewFlowIndicator_fadeOut, 0);

		mCentered = a.getBoolean(R.styleable.CircleViewFlowIndicator_centereds,
				false);
