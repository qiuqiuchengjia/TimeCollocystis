...
public class ViewFlow extends AdapterView<Adapter> {
	// private static final String LOGTAG = LogUtil.makeLogTag(ViewFlow.class);
	...
	private int mScrollMode = SCROLL_MODE_HORIZONTAL;
	private boolean canHandTouch = true;
	...
	public ViewFlow(Context context, AttributeSet attrs) {
		...
		mScrollMode = styledAttrs.getInt(R.styleable.ViewFlow_scrollmode,
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		for (int i = 0; i < count; i++) {
			...
			// Log.d("height", "" + h + "高" + getChildAt(i).getHeight());
			// int hh = getChildAt(i).getHeight();
			// if(hh > mHeight)
			// mHeight = hh/*-18*/;
		}
		...
		if (mFirstLayout) {
			if (mScrollMode == SCROLL_MODE_HORIZONTAL)
				...
				mScroller.startScroll(0, 0, 0, mCurrentScreen * height, 0);
			mFirstLayout = false;
		}
	}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			// HomeMainActivity.INSTANCE.setMenuCanOpen(false);
			/*
		...
		case MotionEvent.ACTION_UP:
			// HomeMainActivity.INSTANCE.setMenuCanOpen(true);
			if (mTouchState == TOUCH_STATE_SCROLLING) {
	...
	protected void onScrollChanged(int h, int v, int oldh, int oldv) {
		...
		if (mIndicator != null) {
			...
			// } else {
			// int vPerceived = v + (mCurrentAdapterIndex - mCurrentBufferIndex)
			// * getHeight();
			// mIndicator.onScrolled(h, vPerceived, oldh, oldv);
			// }
			// TODO
			int hPerceived = h + (mCurrentAdapterIndex - mCurrentBufferIndex)
					* getWidth();
	...
	private void setVisibleView(int indexInBuffer, boolean uiThread) {
		...
		if (mScrollMode == SCROLL_MODE_HORIZONTAL) {
			...
			mScroller.startScroll(mScroller.getCurrX(), mScroller.getCurrY(),
					dx, 0, 0);
			if (dx == 0)
				onScrollChanged(mScroller.getCurrX() + dx,
						mScroller.getCurrY(), mScroller.getCurrX() + dx,
						mScroller.getCurrY());
		} else {
			int dy = (mCurrentScreen * getHeight()) - mScroller.getCurrY();
			mScroller.startScroll(mScroller.getCurrX(), mScroller.getCurrY(),
					0, dy, 0);
			if (dy == 0)
				onScrollChanged(mScroller.getCurrX(),
						mScroller.getCurrY() + dy, mScroller.getCurrX(),
						mScroller.getCurrY() + dy);
		}
	...
	public void setCanHandTouch(boolean can) {
		this.canHandTouch = can;
	}

	public boolean getCanHandTouch() {
		return this.canHandTouch;
	}
