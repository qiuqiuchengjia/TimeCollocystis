...
public class Amount2RMB {
	...
	public static void main(String[] args) {
		...
		System.out.println("RMB:"+convert("101000"));
	}
	/**
	...
	public static  String convert(String amount) throws IllegalArgumentException {
		// amount = numberFormat(Float.parseFloat(amount));
		// System.out.println("numberFormat(Float.parseFloat(amount)):"
		// + numberFormat(Float.parseFloat(amount)));
		String result = "";
		// 去掉分隔符
		amount = amount.replace(",", "");
		String[] str = new String[2];
		if (amount.indexOf(".") != -1) { // 有小数点
			str = amount.split("\\.");
			if (str[1].length() > 1) {// 小数位数大于1位
				amount = str[0] + "." + str[1];
			} else { // 否则
				str[1] = str[1] + "0";
				amount = str[0] + "." + str[1];
