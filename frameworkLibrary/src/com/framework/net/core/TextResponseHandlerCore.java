 ...
 *
 */
public abstract class TextResponseHandlerCore extends TextHttpResponseHandler {
	 ...
	 * @param cache
	 */
	public void sendCacheMessage(byte[] cache, String cacheKey) {
		sendMessage(obtainMessage(CACHE_MESSAGE,
				new Object[] { cache, cacheKey }));
	}
	 ...
	 * @param cache
	 */
	public void onReadCache(byte[] cache, String cacheKey) {
		onCache(getResponseString(cache, getCharset()), cacheKey);
	}

	public abstract void onCache(String cache, String cacheKey);
	...
	protected void handleMessage(Message message) {
		...
		if (message.what == CACHE_MESSAGE) {
			...
			onReadCache((byte[]) response[0], (String) response[1]);
	 ...
	 * @throws IOException
	 */
	public byte[] sendResponseMessageWithCache(HttpResponse response,
			String cacheKey) throws IOException {
		// do not process if request has been cancelled
		if (!Thread.currentThread().isInterrupted()) {
			StatusLine status = response.getStatusLine();
			byte[] responseBody;
			responseBody = getResponsesData(response.getEntity());
	 ...
	 */
	byte[] getResponsesData(HttpEntity entity) throws IOException {
